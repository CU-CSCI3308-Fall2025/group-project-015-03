...
| **Database** | PostgreSQL (tumble_stack) |
...
|--------------|--------|------------------|
| Register with valid inputs | 1. Go to /register page<br>2. Enter unique username w/ a password<br>3. Click Register | User is redirected to /login and info is saved in DB |
| Register with existing username | 1. Attempt to register again using the same username<br>2. Click Register | Website displays error: "Username already exists" and user is not created |
| Submit form with empty fields | 1. Leave one or more fields blank<br>2. Click Register | Website displays form validation error and form does not submit. |
...
| Description | Steps | Expected Outcome |
|--------------|--------|------------------|
| Successful login | 1. Go to /login<br>2. Enter valid credentials<br>3. Click Login | User redirected to /prompts and a new session is created. |
| Invalid password | 1. Enter correct username and incorrect password<br>2. Click Login | Login Fails and return with an error message that says "Invalid username or password" |
| Unregistered user | 1. Enter random username & password<br>2. Click Login | Login fails and return with an error message that says "Invalid username or password" |
...
| Description | Steps | Expected Outcome |
|--------------|--------|------------------|
| Create new entry | 1. Login and go to /journal/new<br>2. Enter title and text & submit | Entry appears in /journal feed |
| Submit empty fields | 1. Login and go to /journal/new<br>2. Leave title or text blank & submit | Website prevents submission and displays an error message |
